...
| Old | New | Migration |
|---|---|---|
| `pybvh.bvh.UpAxis` | `pybvh.Axis` | Rename, and import from the top level. The type never described anything up-specific — `index` / `sign` / `vector` are the same three fields — and it now annotates forward and rest-up axes too. It was not exported in 0.8.0, so only `from pybvh.bvh import UpAxis` breaks. |
| `add_noise(sigma, sigma_pos)` | `add_rotation_noise(sigma)` + `add_position_noise(sigma)` | The two sigmas were in different units — radians for joint angles, the skeleton's length unit for the root — so a single `degrees=` flag had to apply to one and not the other. To reproduce a combined call exactly, chain them with the **same** generator, rotation first: `bvh.add_rotation_noise(s, rng=rng).add_position_noise(p, rng=rng)`. `degrees=` exists only on the rotation form; `wrap=` is unchanged. Both `transforms.*` functions and `Bvh` methods. |
| `FacingFrame(forward, left, up)` | `FacingFrame(forward, left, up, valid)` | `facing_frame` / `Bvh.facing_frame` now also return the per-frame validity mask the construction always computed internally: `valid` is `(F,)` bool, `False` on frames whose basis is the constant fallback rather than a measurement (all-`False` for a skeleton with no L/R pairs — the per-frame refinement of `Bvh.has_lr_geometry`). Attribute access is unchanged; only positional unpacking of exactly three values breaks: `forward, left, up = ...` becomes `forward, left, up, valid = ...`. Fallback rows still carry the usable constant basis rather than `nan`, so rendering-style consumers can ignore `valid`; anything *measuring* facing should mask by it. |
| `skeleton_size` → `1.0` fallback | `skeleton_size` raises `ValueError` | For a skeleton whose size cannot be measured — auto-detection finds no feet, or every foot coincides with the root in the rest pose — `analysis.skeleton_size` / `Bvh.skeleton_size` now raise instead of returning `1.0`. The old substitute read as a plausible metre-scale humanoid, so a caller normalizing by it could not detect the fallback and silently produced unnormalized data. Migration: pass `foot_joints=` explicitly, or catch the `ValueError` and choose your own scale. `foot_contacts`' *internal* threshold scale keeps a private `1.0` fallback — a threshold must exist even where a measurement does not. |

### Added

- `analysis.dimensionless_jerk` / `analysis.log_dimensionless_jerk` take `normalize={"peak_speed", "mean_speed", "amplitude"}`, selecting which of the normalizers in Hogan & Sternad 2009 divides the jerk integral. `"peak_speed"` stays the default, so existing calls are unchanged; the sign convention (negative is less smooth) is the same for all three. `"amplitude"` additionally requires the movement extent via `amplitude=` (scalar, or `(K,)` for a `(T, K)` speed input), since a speed profile alone cannot tell you the endpoint displacement. Both options forward through `analysis.smoothness(...)` and `Bvh.smoothness(...)`.

  `"mean_speed"` and `"amplitude"` are the same measure — `A = v_mean · D` — so `"mean_speed"` is `"amplitude"` evaluated at the arc length the speed profile implies, `A = ∫|v| dt`. Passing `amplitude=` is a different measure only when the extent comes from elsewhere, typically the endpoint displacement. `"peak_speed"` is genuinely distinct: it differs by a factor of `speed_metric(v)**2`, which depends on the shape of the speed profile rather than being constant. The docstring now states which convention the default implements, so a mismatch against a published DLJ figure is identifiable as a convention difference rather than a bug.

  Passing `amplitude=` alongside `normalize="peak_speed"` / `"mean_speed"` raises rather than being silently dropped.

- `transforms.add_rotation_noise` / `Bvh.add_rotation_noise` take `degrees=False`, matching `rotate_vertical` / `random_rotate_vertical` and the rest of the angle-taking API. `add_position_noise` deliberately has no such flag — a translation has no angular unit.

- `io.write_bvh_file` / `Bvh.write` take `overwrite=True`. The default preserves existing behavior; `overwrite=False` raises `FileExistsError` instead of replacing a file, and does so before the file is opened, so the destination is left intact.

- `pybvh.parse_axis(axis, *, allow_unsigned=False)` — the signed-axis parser behind `Bvh.up_axis`, now public, returning a `pybvh.Axis`. For code holding an axis string of its own (a dataset convention from a config, or a string pybvh returned) instead of re-deriving `s[-1]` / `s[0] == '-'` by hand. Unlike the internal helpers it validates first, so bad input raises `ValueError` rather than `IndexError`. `allow_unsigned=True` accepts a bare `'x'` as `'+x'`.

- `Bvh.forward_axis` and `Bvh.rest_up_axis` join `Bvh.up_axis`, parsing `rest_forward` and `rest_up`. Each mirrors the nullability of the string it parses: `rest_up_axis` is `Axis | None` (a degenerate rest pose carries no direction), the other two are always defined.

- `rotations.quat_unwrap(q, axis=0)` — flips signs along a sequence axis so consecutive quaternions stay in one hemisphere. `rotmat_to_quat` (and so `Bvh.to_quat` and `to_feature_array(representation="quat")`) applies the `w >= 0` canonical form per element, which is *not* temporally continuous: a joint turning through 180° flips sign between adjacent frames while the motion is smooth. The rotations were always exact — the representation jumped — but anything differencing or measuring distance on the raw values saw a discontinuity that is not in the motion. All three call sites now document the canonical form and point here.

- `rotations.quat_slerp(..., shortest=True)` — selects which of the two arcs between the endpoints to travel. The default is the existing behavior (never turn more than 180°, which is what `Bvh.resample` needs); `shortest=False` preserves a turn that genuinely exceeds 180°, such as a wind-up or a spin, which the short arc silently shortens.

- `analysis.number_of_peaks(speed, min_height=None)` — minimum height for a maximum to count. The default is unthresholded, as before and as in the source literature; the docstring now states that this makes the count track noise rather than movement structure on noisy data, and that an exactly flat maximum counts as zero peaks under the strict comparison.

- The golden smoothness fixture pins `sparc` at three non-default parameter sets as well as the default. `padlevel` / `fc` / `amp_th` are the whole reason that function takes parameters and had no reference coverage at all — the same blind spot that hid the dimensionless-jerk normalizer divergence. A companion test asserts the pinned sets actually produce different answers, so a row that could not detect its parameter being ignored fails rather than passing quietly. The previously committed numbers are unchanged and were verified to reproduce bit-identically.

- `Bvh.has_lr_geometry` — whether the skeleton's L/R pairs give a usable lateral axis, i.e. whether `rest_forward` / `forward_axis` / `forward_at` / `left_at` are *measuring* this skeleton or falling back to an arbitrary-but-stable axis derived from `world_up` alone. The fallback is indistinguishable from a measurement in those return values, which matters most for the check `rest_forward`'s own docs suggest: comparing a skeleton's facing against a dataset convention would otherwise "match" every time for a skeleton with no L/R pairs. It is `False` exactly when deriving a facing emits the fallback warning below — including on a file whose rest-pose and animation up axes disagree, where the L/R pairs exist but their axis is parallel to `world_up` and so cannot give a facing.

- `signal.fft_magnitude(..., norm=)` — normalization of the returned magnitude spectrum. `"backward"` (default, the existing raw `|rfft|`), `"ortho"` (`/√N`) and `"forward"` (`/N`) follow numpy's `rfft` vocabulary; `"amplitude"` is the single-sided amplitude spectrum the signal-processing literature plots — `2|X|/N` with the DC and Nyquist bins not doubled, so a pure sine of amplitude `A` peaks at `A`. `"amplitude"` is deliberately *not* one of numpy's norms: numpy's `"forward"` lacks the one-sided doubling and reads half the sine's amplitude, which is exactly the mismatch the option exists to close. `dominant_frequency` is argmax-based and unaffected.

- `geometry.pose_distance(..., reduction=)` — `"frobenius"` (default, the existing `‖X₁ − X₂‖` over all joints and coordinates) or `"mpjpe"`, the mean per-joint position error of the pose-estimation literature (mean over joints of per-joint Euclidean error). The two differ by the constant `√N` only when per-joint errors are uniform and diverge beyond any constant otherwise — Frobenius weights large per-joint errors quadratically — so a published MPJPE figure cannot be recovered from the Frobenius value; now it does not have to be.

- `analysis.relative_scale_factor(..., centered=False)` — `centered=True` subtracts each array's centroid before the least-squares fit, the Procrustes/Umeyama centering convention, which is what a world-frame sequence with a translated root needs (the default origin-anchored fit is corrupted in proportion to the centroid offset — right for rest poses with the root at the origin, and previously the only option, with the failure mode only described in the docstring). Scale-only: no rotation is estimated, so this is not the full Umeyama similarity fit.

- `foot_contacts(floor="min")` / `ground_contacts(floor="min")` — the floor as the true minimum of the per-frame minimum foot height, joining `"auto"` (the robust 2nd percentile) and an explicit float. Exact where `"auto"` is robust: a single glitched-low frame drags `"min"` down with it, which is also why `"min"` never fills or serves the cached `Bvh.floor_height` (that property stays 2nd-percentile canonical). Extending the existing `floor=` vocabulary instead of adding a `floor_percentile=` knob keeps one parameter as the single answer to "where does the floor come from", with no ignored-argument combinations.

- `transforms.rotate_vertical(pivot=)` / `random_rotate_vertical` / `Bvh` methods / the array-level `rotate_angles_vertical` — the world-space point the motion turns about. `"origin"` (default, the existing behavior: a motion captured away from the origin sweeps along an arc), `"root"` for turn-in-place about the first frame's root position, or an explicit `(3,)` landmark. Only the two horizontal components of a point are read, since every point on a vertical line spans the same rotation axis, so heights come through untouched; joint angles — including the root's world rotation — are pivot-independent, as the pivot translates the root trajectory and nothing else. `"root"` is exactly *center on the first-frame root → rotate about the origin → un-center*, so a pipeline that already centers its clips gets turn-in-place from the default. No random-pivot variant: a random pivot is `rotate_vertical` composed with `translate_root`, and the distribution to draw from is a pipeline's decision.

- `batch.read_bvh_directory(sort="natural")` — orders embedded digit runs numerically (`clip2.bvh` before `clip10.bvh`, case-insensitive). Opt-in: the default stays lexicographic (`sort=True`, also spellable `sort="lexicographic"`) because it matches what plain `sorted()` produces elsewhere — silently switching would misalign label lists and split manifests built independently with `sorted()`.

### Changed

- `analysis.cadence` and `analysis.walking_pace` (and the same fields of `gait_parameters`) return `nan` on a zero-duration clip, where they previously returned `0.0`. A rate over no time is *undefined* — `0.0` claimed "not stepping", which is a measurement — and the other six `GaitParameters` fields already said `nan` for their underdetermined cases, so consumers aggregating gait parameters had to be nan-aware regardless. Zero onsets over a *positive* duration is still a genuine `0.0`.

- `analysis.speed_metric` takes the magnitude on both sides — `mean|v| / max|v|` rather than `mean(v) / max|v|`. Identical for any non-negative speed profile. For a signed scalar velocity — a supported input, since these measures are defined on `x(t)`, "any scalar coordinate" — the raw mean cancels toward zero on an out-and-back movement and produced a ratio outside the documented `[0, 1]`. This also makes `dimensionless_jerk(v, fs, "mean_speed") == dimensionless_jerk(v, fs) / speed_metric(v)**2` exact for signed input.

- The smoothness kernels now state what each does with sign. Both input kinds are valid and appear in the source literature: a non-negative speed profile (what `Bvh.smoothness` feeds) and a signed scalar velocity (what Hogan & Sternad define the measures on). The rule, now documented per function rather than left to each kernel: *normalizers* take magnitudes (`max|v|`, `mean|v|`), while the signal being differentiated or spectrum-analysed is used as given.

- Deriving a facing direction now warns when it cannot measure one. With no usable left/right geometry, `rest_forward` / `forward_axis` / `forward_at` / `left_at` / `facing_frame` fall back to an arbitrary-but-stable horizontal axis chosen from `world_up` alone, and the returned axis cannot say so. A `UserWarning` now does, naming both remedies (`bvh.lr_mapping = {...}` to fix, `bvh.has_lr_geometry` to detect). This matches the existing `world_up` inference warning; Python's default filter shows it once per call site, so per-frame consumers stay quiet.

- `world_up` warns when it has to invent an answer. A skeleton with no recognizable `head`/`neck`/`chest`/`spine` joint *and* a degenerate rest pose previously got `'+y'` silently — a guess that the returned string cannot distinguish from a measurement, and that everything downstream of up (floor height, foot contacts, facing, ground-plane projections) then rests on. Same treatment and same wording style as the two warnings above.

- Docstrings across the public surface now name the convention they implement, its published alternative, and where the two diverge — the rule added to `CLAUDE.md` this release, applied by sweeping every module rather than only the functions that prompted it. No behavior changes and no signatures changed; the choices were already made, they were just not stated where a user reads them. The ones most likely to have cost someone a debugging session: `angular_velocities` returns rates of the *parent-relative* rotation in the body frame (not the world-frame segment rates of the biomechanics convention); `to_feature_array`'s velocity block is units per **frame** while `joint_velocities` defaults to units per second; `read_bvh_file` snaps a truncated `Frame Time` to an exact `1/N`, reorders root channels to position-first, and `write_bvh_file` quantizes to 6 decimals, so a round-trip is lossless in motion but not byte-for-byte; `temporal_stats` uses population estimators where pandas and scipy default to bias-corrected ones; `fft_magnitude` is unnormalized and length-dependent; `relative_scale_factor` fits about the origin, not Procrustes-centered; `cadence` is steps per second where the gait literature reports steps per minute; and `gait_parameters` documents `stride_cv`'s within-foot pooling, `asymmetry`'s unsigned-and-unitless form against Robinson's signed ×100 Symmetry Index, and the fact that `stride_length` includes step width while `step_length` projects onto a single whole-clip travel chord.

### Fixed

- `mirror`'s docstring described the `lateral_axis=None` auto-detection as picking "the axis that is neither forward nor upward". It does not: it averages left-minus-right rest-pose offsets over the L/R joint pairs and takes the dominant axis, measuring how the skeleton is actually built rather than assuming a convention. The old wording led at least one downstream to hand-roll a weaker replacement for a default that was already better.

- `rotmat_to_euler`'s docstring said gimbal lock is resolved by zeroing the third angle. It zeroes the **first** and folds the residual into the third — the opposite of what was written, and the opposite of what SciPy's `Rotation.as_euler` does, so anyone cross-checking a locked decomposition against SciPy was reading a docstring that endorsed SciPy's answer while the code returned the other split. Both describe the input rotation exactly and round-trips were never affected, which is why the contradiction survived. The docstring now also states the returned angle ranges and that a triple authored outside them round-trips to its in-range equivalent rather than to itself.

- `facing_frame` described `root_trajectory`'s heading as a travel direction ("facing is where the body points, heading is where it moves"). It is not — that heading is the root bone's rotation applied to the rest forward, with no velocity in it, so side-stepping does not separate the two. Both docstrings now give the real distinction: two facing estimates built differently, one inheriting pelvis twist and pitch, the other whole-body and yaw-only, diverging whenever the pelvis turns relative to the body.

- `perturb_speed` documented a return with "adjusted frame count and `frame_time`". It preserves the original `frame_time` — the clip's duration changes, its playback rate does not.
...
One row per rename / behavior change relative to v0.7.0.
...
- **`pybvh.analysis` motion primitives** — jerk (`node_jerk`, `joint_jerk`); smoothness on a 1-D speed profile (`sparc`, `dimensionless_jerk`, `log_dimensionless_jerk`, `number_of_peaks`, `speed_metric`, `integrated`/`mean`/`rms_squared_jerk`, and a `smoothness(metric=…)` dispatcher; SPARC/DLJ/LDLJ validated against the Balasubramanian reference); signal reductions (`velocity_reductions` — a `(peak, mean, peak_to_mean, peak_acceleration, peak_deceleration)` named tuple with both directional rates clamped `>= 0` — plus `zero_crossings`, `active_segments`, `active_duration`; time-based kernels take a required `fs`); `kinetic_energy`; gait (`cadence`, `stride_length`, `walking_pace`); `range_of_motion`; and covariance descriptors (`cov3dj`, `lagged_covariance` — mean-centered, population convention).
- **`pybvh.analysis.skeleton_size`** (absolute mean root-to-foot scale, the public name for the scale `foot_contacts` uses internally; unknown explicit joint names raise, and the `1.0` degenerate fallback applies only to footless auto-detection) and **`pybvh.analysis.relative_scale_factor`** (least-squares uniform scale between two poses; exported as `pybvh.relative_scale_factor`) — disambiguated size primitives.
...
- **`node_speed_derivative` / `joint_speed_derivative`** (and the `Bvh` wrappers) — the per-frame rate of change of speed `d‖v‖/dt`, one more rung on the finite-difference ladder with the exact `node_accelerations` signature (`centered` / `in_frames` / `coords=` / `stencil` / `pad`, applied consistently at both stages). Positive = speeding up, negative = slowing down; this is the tangential acceleration `a_t` (the normal component is `‖v‖²·κ` via `geometry.curvature`; `a_t² + a_n² = ‖a‖²`) and is **not recoverable from `node_accelerations`** — the delta-of-norm is not the norm-of-delta (a direction change at constant speed has `‖a⃗‖ > 0` but `d‖v‖/dt = 0`). `velocity_reductions`' `peak_acceleration`/`peak_deceleration` are exactly the clamped extrema of this series under `stencil="forward"`, `pad="none"` (asserted in the test suite). Source: Hachimura et al. 2005 (Time Effort).
- **Reduction kernels accept a trailing profile axis** — `velocity_reductions`, `sparc`, `smoothness` and every metric it dispatches to (`dimensionless_jerk`, `log_dimensionless_jerk`, `integrated`/`mean`/`rms_squared_jerk`, `number_of_peaks`, `speed_metric`), plus `active_duration`, take either a 1-D `(T,)` profile (scalar outputs) or a `(T, K)` array — `K` speed profiles in columns, reduced independently per column to `(K,)` outputs (`VelocityReductions` fields become `(K,)` arrays). Degenerate columns go `nan` per column; inputs of rank > 2 raise `ValueError` naming the accepted shapes.
- **`facing_frame` — the per-frame facing basis as vectors** (`Bvh.facing_frame()` / `analysis.facing_frame`) — returns a `FacingFrame(forward, left, up)` named tuple of three `(F, 3)` unit-vector arrays: the continuous, yaw-only (gravity-aligned) orthonormal basis (`forward × left = up`) that `forward_at` / `left_at` snap to a signed axis label. The snapped label is lossy in exactly the way that hurts downstream body-frame math: on the bundled CMU turning walk it reads `'+z'` on every frame while the true facing rotates ~16° — the vectors track the rotation (requested by pybvh-qualities, Request C; the label API is unchanged). Frames with no usable L/R direction receive the same fallback chain `forward_at` documents, expressed as vectors, so the triple stays orthonormal. Internally the leftward/facing geometry is consolidated into a single vectorized implementation in `pybvh.tools` shared by `forward_at` / `left_at`, `facing_frame`, and the bvhplot follow camera (whose outputs are pinned unchanged in the test suite). Distinct from `root_trajectory`'s travel heading — facing is where the body points, heading is where it moves.
- **`kinetic_energy` accepts `masses` as a `(J,)` array or a `{joint_name: mass}` mapping** — the mapping form is order-independent and validated for exact coverage (clear error on a missing/unknown joint or a wrong-length array), and any masses vector must have a positive total (`ValueError` otherwise — an all-zero mass vector would silently zero the energy). `masses=None` keeps the unit-mass proxy.
...
- **`Bvh.up_axis`** — `world_up` parsed into a `UpAxis(index, sign, vector)` named tuple, so consumers stop re-parsing the signed axis string by hand: `coords[:, :, bvh.up_axis.index] * bvh.up_axis.sign` is the up-positive height. `vector` is a fresh unit array per access. The internal parse sites (`foot_contacts`, `auto_detect_foot_joints`, the `floor_height` estimator, `reorient_rest_forward`, `rotate_vertical`) all share the same parser.
- **`analysis.ground_contacts(bvh, joints, ...)` + `Bvh.ground_contacts`** — the `foot_contacts` detection engine opened up to arbitrary joint sets (hands during floor work, knees, props; `joints` takes node names and/or node-space indices, end sites included, negative indices allowed). `foot_contacts` runs on the same shared core, so the two cannot drift. The foot-presuming behaviors are deliberately dropped: no rest-pose feet-below-hips sanity check, no `Bvh.floor_height` cache interaction, and `height_reference` defaults to `"floor"` — the `"velocity"` stance calibration presumes regular ground contact, which an arbitrary joint set doesn't have (its docstring carries a prominent warning that `floor="auto"` estimated from a never-grounding joint set fabricates contacts; pass `floor=bvh.floor_height`).
- **`rotations.mean_rotation`** — chordal (Frobenius) mean of a batch of rotation matrices `(..., N, 3, 3) → (..., 3, 3)` via SVD projection with a determinant guard (always a proper right-handed rotation). Closed-form, batch-vectorized, no weights; the docstring documents the wide-spread degeneracy (rank-collapsed means are valid but arbitrary) and points at `rotation_geodesic_distance` for a spread check. The natural companion to `facing_frame` for averaging a clip's facing.
- **`Bvh.joint_tips`** — `{joint_name: end-site node index | None}`, the bone-tip lookup that previously required manual tree walking. Resolution is identity-based on the node tree, never by name: end-site display names are cosmetic and can collide with real joint names, which would silently corrupt a `node_index`-based lookup. A joint with several end sites (nonstandard, but the parser accepts it) maps to the first in file order.
...
- **`foot_contacts(foot_joints=[])` raises a clear `ValueError`** ("contact detection needs at least one joint") instead of crashing deep inside the floor estimator with a zero-size reduction; `ground_contacts` shares the same guard. The footless-rig contract is documented end to end: `auto_detect_foot_joints` returns `[]` (never raises) on footless rigs, and the detectors are where emptiness becomes fatal.
