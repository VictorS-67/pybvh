...
- **`reorient_rest_forward()` succeeds on more skeletons** — semantic delta from the L/R mapping rewire: skeletons whose source forward direction was previously underivable (because `_compute_forward_at` was transitively name-dependent via the old `_rest_lateral`) now succeed when `bvh.lr_mapping` is populated by either extended strategy A or an explicit user mapping. Behaviour change is intentional and tested.
- **`_rest_lateral` returns `None` instead of raising** when no L/R pairs are available. Internal helper; external callers that catch the previous `ValueError` should switch to `is None` checks. Used by `mirror()`, `_compute_forward_at`, and (transitively) `forward_at()`.
- **`get_forw_up_axis()` and `get_up_axis_index()` removed** from `pybvh.tools` — replaced by the `Bvh.world_up` property, `Bvh.forward_at()` method, and internal helpers (`_rest_upward`, `_rest_lateral`, `_infer_world_up`, `_compute_forward_at`, `_world_lateral_unit_at_frame`, `_signed_rotation_delta_around_axis`). The old functions were internal-only, so no external deprecation shim. Callers inside pybvh that used the old functions (`bvhplot._common.get_camera_angles`, `transforms.mirror`, `transforms.rotate_vertical`, `features.root_trajectory`, `features.foot_contacts`, `bvhplot._matplotlib.trajectory_mpl`) have been updated to use the new API.
