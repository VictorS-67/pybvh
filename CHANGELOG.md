...
- **Reduction kernels accept a trailing profile axis** — `velocity_reductions`, `sparc`, `smoothness` and every metric it dispatches to (`dimensionless_jerk`, `log_dimensionless_jerk`, `integrated`/`mean`/`rms_squared_jerk`, `number_of_peaks`, `speed_metric`), plus `active_duration`, now also accept `(T, K)` input — `K` speed profiles in columns, reduced independently per column to `(K,)` outputs (`VelocityReductions` fields become `(K,)` arrays). The 1-D `(T,)` → scalar behavior is bit-identical to before (the SPARC FFT is batched along the time axis with the same `nfft`); degenerate columns go `nan` per column; inputs of rank > 2 raise `ValueError` naming the accepted shapes.
...
- **`Bvh.up_axis`** — `world_up` parsed into a `UpAxis(index, sign, vector)` named tuple, so consumers stop re-parsing the signed axis string by hand: `coords[:, :, bvh.up_axis.index] * bvh.up_axis.sign` is the up-positive height. `vector` is a fresh unit array per access. The four internal parse sites (`foot_contacts`, `auto_detect_foot_joints`, the `floor_height` estimator, `reorient_rest_forward`) now go through it, and `rotate_vertical` shares the same `tools`-level parser — behavior pinned bit-identical by the golden foot-contacts fixture.
- **`analysis.ground_contacts(bvh, joints, ...)` + `Bvh.ground_contacts`** — the `foot_contacts` detection engine opened up to arbitrary joint sets (hands during floor work, knees, props; `joints` takes node names and/or node-space indices, end sites included, negative indices allowed). `foot_contacts` itself is now a thin wrapper over the same shared core, so the two can never drift. The foot-presuming behaviors are deliberately dropped: no rest-pose feet-below-hips sanity check, no `Bvh.floor_height` cache interaction, and `height_reference` defaults to `"floor"` — the `"velocity"` stance calibration presumes regular ground contact, which an arbitrary joint set doesn't have (its docstring carries a prominent warning that `floor="auto"` estimated from a never-grounding joint set fabricates contacts; pass `floor=bvh.floor_height`).
...
- ⚠️ **Zero-total weights/masses raise.** `geometry.center_of_mass(pts, weights=)` rejects a weight vector without a positive total (zero, sub-epsilon, negative, or NaN sum) — previously it silently returned all-NaN positions for every frame; individual negative weights with a positive total stay allowed. `kinetic_energy` (and anything routing through the masses resolver) likewise rejects masses that don't sum to `> 0` — an all-zero mass vector silently zeroed the energy. The unit-mass `masses=None` default and the `weights=None` centroid are unaffected. The `nan` policy remains for *data* degeneracy; invalid *arguments* now raise.
...
- **The scalar reduction kernels no longer silently mis-reduce 2-D input.** `velocity_reductions` accepted a `(T, K)` array and reduced it globally (one max/mean over all elements); `sparc` FFT'd, and the jerk-based metrics differenced, along the wrong (last) axis; `active_duration` summed active samples across columns — all plausible-looking garbage for multi-effector input. A `(T, K)` array now produces correct per-column results (see Added), and anything of higher rank raises `ValueError`.
...
- **`foot_contacts(foot_joints=[])` raises a clear `ValueError`** ("contact detection needs at least one joint") instead of crashing deep inside the floor estimator with a zero-size reduction; `ground_contacts` shares the same guard. The footless-rig contract is now documented end to end: `auto_detect_foot_joints` returns `[]` (never raises) on footless rigs, and the detectors are where emptiness becomes fatal. Both `height_reference` constants are fully documented (`"velocity"` = per-foot stance-median calibration, `"floor"` = fixed `0.013 × skeleton_scale` margin) — previously the `"floor"` semantics were only discoverable from source.
